...
inline void InflationExtraction::assign_statistics_data(
    ...
    const amrex::Vector<amrex::Real> data, const int component,
    const int num_comps, const amrex::Vector<int>::const_iterator itr,
                                          ...
                                          const int component)
{
    ...
    amrex::Real dkiso =
        sqrt(3.) * 2. * amrex::Math::pi<amrex::Real>() / inflt_methods.L;
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k)
        {
            ...
            for (int s = 1; s <= cfg.N / 2; s++)
            {
                ...
                {
                    amrex::Real power =
                        (std::pow(field_arrs[bx](i, j, k, component).real(),
                                  2.0) +
                         std::pow(field_arrs[bx](i, j, k, component).imag(),
                                  2.0));

                    int comp =
                        (kmag == kiso_ptr[cfg.N / 2]) ? cfg.N / 2 : s - 1;
...
inline amrex::Real InflationExtraction::calculate_field_moment_x(
    const amrex::MultiFab &field, const amrex::Vector<amrex::Real> mean,
                                  ...
                                  SmallDataIO &file, const int is_first_step)
{
    ...
    // Find iterators, which determine which moments are requested and their
    // ordering
    amrex::Vector<int>::const_iterator start = moment_orders.begin();
    amrex::Vector<int>::const_iterator mean_itr =
        std::find(moment_orders.begin(), moment_orders.end(), 1);
    amrex::Vector<int>::const_iterator stdev_itr =
        std::find(moment_orders.begin(), moment_orders.end(), 2);
    amrex::Vector<int>::const_iterator skew_itr =
        std::find(moment_orders.begin(), moment_orders.end(), 3);
    amrex::Vector<int>::const_iterator kurt_itr =
        std::find(moment_orders.begin(), moment_orders.end(), 4);
    ...
    for (int comp = 0; comp < nc; comp++)
    {
        ...
        if (mean_itr != moment_orders.end())
        {
            assign_statistics_data(headers, names[comp] + "-mean",
                                   data_to_print, means, comp, nc, mean_itr,
                                   start, is_first_step);
        }
        ...
        if (moment_orders.back() != 1)
        {
            ...
            {
                assign_statistics_data(headers, names[comp] + "-stdev",
                                       data_to_print, stdev, comp, nc,
            ...
            if (moment_orders.back() != 2)
            {
                ...
                {
                    kurt[comp] =
                        calculate_field_moment_x(field, means, 4, comp);
...
inline void InflationExtraction::extract(const amrex::MultiFab &state)
{
    ...
    amrex::MultiFab out_MF(hs_x.boxArray(), hs_x.DistributionMap(),
                           output_comps, 0);
    ...
    SmallDataIO stats_file(m_data_path + "field-statistics", dt, time,
                           restart_time, SmallDataIO::APPEND, first_step,
                           ".dat");

    if (!orders.empty())
    {
        stdevs =
            print_moment(out_MF, var_names, orders, stats_file, first_step);
    }
    ...
    if (std::find(orders.begin(), orders.end(), 2) != orders.end())
    {
        ...
        const std::vector<amrex::Real> tsr_data = {stdevs[1] / stdevs[0],
                                                   stdevs[2] / stdevs[0]};
