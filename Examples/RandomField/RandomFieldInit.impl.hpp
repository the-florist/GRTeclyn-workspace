...
{
    if (!inflt_methods.read_from_stoiic)
    {
        return;
    }
...
RandomFieldInit::find_in_stoiic(const InflationConfig &cfg,
                                const amrex::Real km,
                                ...
                                const auto field_selector)
{
    ...
    {
        if (amrex::Math::abs(km - cfg.init_k_ptr[idx]) <
            InflationUtils::tolerance)
        {
            spec_index = idx;
            break;
        }
                                        ...
                                        const auto field_selector)
{
    ...
    {
        if constexpr (std::is_same_v<std::decay_t<decltype(field_selector)>,
                                     ScalarField>)
        {
            AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
                cfg.read_from_stoiic,
                "RandomFieldInit::calculate_random_field, scalar de-Sitter "
                "ICs are not yet implemented");
        }
        else
        {
            value = calculate_mode_function(cfg, kmag, field_selector);
        }
    }
        ...
        [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k)
        {
            ...
            if (cfg.scalar_init)
            {
                ...
                constexpr amrex::GpuArray<ScalarField, 4> all_scalar_fields{
                    ScalarField::Phi, ScalarField::Pi, ScalarField::Chi,
                    ScalarField::K};
    ...
    amrex::MultiFab &scalar_fields_x, const int dN)
{
    ...
    if (inflt_methods.scalar_init)
    {
        ...
        amrex::Print() << find_precision_loss(scalar_fields_x, 0,
                                              inflt_methods.phi0)
                       << "\n";
