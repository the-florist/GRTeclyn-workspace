...
class Potential
{
  public:
    struct params_t
    {
        ...
        void fill_params()
        {
            ...
            if (type != 8)
            {
                potential_pp.get("param_1", scalar_mass);
            }
            switch (type)
            {
            // quadratic
        ...
        void check_params() const
        {
            ...
            else if (type == 10)
            {
                AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
                    ...
                    "Potential::punctuated, punctuated inflation parameters "
                    "uninitialised");
            }
            else
            {
  ...
  public:
    ...
    monodromy(data_t &V, data_t &dV, const data_t &phi) const
    {
        ...
        amrex::Real displaced_argument =
            (m_params.location - phi + m_params.width) / m_params.period;

        amrex::Real envelope =
            0.25 * (1. + tanh(argument)) * (1. + tanh(displaced_argument));
        ...
        // Calculate dV
        amrex::Real d_envelope =
            0.25 / m_params.period *
            ((1. + tanh(argument)) *
                 (std::pow(tanh(displaced_argument), 2.) - 1.) +
             (1. + tanh(displaced_argument)) *
                 (1. - std::pow(tanh(argument), 2.)));
        amrex::Real d_oscillation = -sin(argument) / m_params.period;

        dV  = pow(m_params.scalar_mass, 2.0) * phi;
        dV += m_params.amplitude *
              (envelope * d_oscillation + d_envelope * oscillation);
    }

    // Prokopec USR model, from arXiv:2507.04114
    template <class data_t>
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void USR(data_t &V, data_t &dV,
                                                      const data_t &phi) const
    {
        // Calculate V
        amrex::Real fraction =
            (3. * pow(phi, 2.) + 2. * sqrt(2.) * phi * m_params.v +
             6. * pow(m_params.v, 2.));

        fraction /= pow(3. * pow(phi, 2.) + 2. * pow(m_params.v, 2.), 2.);
        V = m_params.Lambda * pow(m_params.v, 4.) * pow(phi, 2.) * fraction /
            3.;
                      ...
                      const vars_t<data_t> &vars) const
    {
        ...
        default:
            AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
                false, "Potential::compute_potential, "
                                 ...
                                 const data_t phi) const
    {
        ...
        default:
            AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
                false, "Potential::compute_background_potential, "
