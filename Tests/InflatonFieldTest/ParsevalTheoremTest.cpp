...
void run_parseval_theorem_test()
{
    ...
    amrex::Initialize(amrex_argc, amrex_argv);
    {
        ...
        // amrex::FFT::R2C::forward is unnormalised, as FFTW is, so it returns
        // H_k = sum_x h_x exp(-2 pi i k.x / N) and satisfies
        //     sum_k |H_k|^2 = N^3 sum_x |h_x|^2.
        // DerivedVariables divides by N^3 before using any Fourier-space
        // field, so do the same here: in those units Parseval's theorem reads
        //     sum_k |H_k / N^3|^2 = sum_x |h_x|^2 / N^3,
        // which is the pair of quantities compared below. Without this the two
        // sides differ by a factor of N^6.
        hk.mult(std::pow(num_cells, -3.), 0, 1);
